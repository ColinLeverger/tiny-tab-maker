...
(async function () {
  ...
  assert.strictEqual(sync.configured(), true);
  assert.strictEqual(sync.config().repo, "owner/private-data");
  assert.strictEqual(sync.config().token, "token");
  assert.strictEqual(sync.config().path, "");
  await sync.syncNow();
